...
public class World {
	...
	/**
	 * Constructor of the World class. It sets the current level and loads that
	 * level. It initializes the arrays for the sprites to be hidden, the
	 * sprites to be restored, histories, copy of sprites.
	 * 
	...
	/**
	 * This method is a getter which returns the index of the sprites array that
	...
	/**
	 * This method stores the positions of players and specific blocks and uses
	 * their copy constructors to store information about the state of those
	 * sprites in the movesHistory array.
	 * 
	 * @throws SlickException
	 */
	public static void recordMovesHistory() throws SlickException {
		for (int j = spritesCopy.length - 1; 0 <= j; j--) {
			if (!spritesCopy[j].getShow()) {
				continue;
			}

			if (spritesCopy[j] instanceof Ice) {
				Ice iceCopy = new Ice((Ice) spritesCopy[j]);
				movesHistory.add(new SpriteMove(((Player) getSprite("Player"))
						.getMoves(), j, iceCopy));
			} else if (spritesCopy[j] instanceof Stone) {
				Stone stoneCopy = new Stone((Stone) spritesCopy[j]);
				movesHistory.add(new SpriteMove(((Player) getSprite("Player"))
						.getMoves(), j, stoneCopy));
	...
	public void undo() throws SlickException {
		...
		if (((Player) getSprite("Player")).getMoves() == 0) {
			return;
		}
		...
		if (((Sprite) getSprite("Tnt")).getShow() == false) {
			tntExploded = true;
		}
		/*
		 * Revert all the Sprite objects that are of the last index based on the
		 * movesHistory array.
		 */
		for (int j = movesHistory.size() - 1; 0 <= j; j--) {
			...
			if (movesHistory.get(j).getMoveIndex() == (((Player) getSprite("Player"))
					.getMoves() - 1)) {
				if (movesHistory.get(j).getSprite().getClass()
						.equals(Stone.class)) {
		...
		while (true) {
			...
			if (movesHistory.get(tempIndex).getMoveIndex() == ((Player) getSprite("Player"))
					.getMoves()) {
	 ...
	 * 
	 * @param newWillRestart
	 *            boolean variable that shows whether to restart or not.
	 */
	 ...
	 * corresponding to that variable that has been incremented. It also resets
	 * the move history made by the blocks and player.
	 * 
	 * @throws SlickException
	 */
	...
	/*
	 * This method checks whether the level has been completed or not. It checks
	 * whether all the targets are covered by blocks or not. It returns true if
	 * all targets are covered by blocks, false otherwise.
	 */
	 ...
	 * sprite. Note that the coordinates of the player sprite is only accessible
	 * in the sprites array that is initialised in the World class.
	 * 
	 ...
	 * 
	 * @param className
	 *            the class that wants to be checked.
	 * @return true if a sprite of that class exists in the toHide array, false
	 *         otherwise.
	 */
	...
	/**
	 * This method returns the index of a particular sprite based on its sprite
	 * type. This is done with the assumption that there is only one door and
	 * one switch on each map.
	 * 
	 * @param className
	 *            generic Class<T>
	...
	/**
	 * This method checks whether a sprite is blocked by another particular
	 * sprite.
	 * 
	 * @param x
	 *            x coordinate in the map
	...
	/**
	 * This method takes the x and y coordinates of a particular position and
	 * returns whether the position is occupied by a sub-class of the
	 * Inpenetrable class.
	 * 
	 * @param x
	 *            x coordinate in the map.
	...
	public static boolean isBlocked(String tag, float x, float y) {
		...
		/*
		 * No need bound checking because the wall is eventually will become the
		 * sign of the bounds?
		 */
		for (int j = spritesCopy.length - 1; 0 <= j; j--) {
			if (!spritesCopy[j].getShow()) {
	 ...
	 * @return true if the current coordinate is blocked by an adjacent block,
	 *         false otherwise.
	 */
